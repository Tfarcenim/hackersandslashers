...
public class AttackSpark extends TextureSheetParticle {
   ...
   public AttackSpark(ClientLevel world, double x, double y, double z, double vx, double vy, double vz, SpriteSet spriteSet) {
      ...
      double spread = 0.5D;
      this.xd = (this.random.nextDouble() - 0.5D) * spread;
      this.yd = (this.random.nextDouble() - 0.5D) * spread;
      this.zd = (this.random.nextDouble() - 0.5D) * spread;
      this.rCol = 1.0F;
      this.gCol = 1.0F;
      this.bCol = 0.25F;
   ...
   public void tick() {
      ...
      if (this.age < this.lifetime) {
         ...
         this.roll += (float)Math.toRadians((double)currentRollSpeed);
      }
